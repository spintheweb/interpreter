...
import Group from './elements/Group.mjs';

const ROOT_DIR = process.cwd();
const SITE_DIR = path.join(ROOT_DIR, 'public');

export default function stwSpinner(app, webbase) {
    // Load webbase
    Webbase(app, path.join(SITE_DIR, webbase));
    ...
    app.ws('/', (ws, req) => {
        ...
        app.on('request', (req, res) => {
            let requested = app.webbase.Render(req);

            readFile(requested, (err, data) => {
                if (err) // If the request is not a file than it could be a webbase element (API request)
                    res.end(requested);
                else {
                    res.writeHead(200, { 'content-type': lookup(requested) });
                    res.end(data);
                }
            })
        ...
        ws.on('connection', socket => {
            ...
            socket.onmessage = socket => {
                ...
                try {
                    ...
                    socket.data.url = new URL('http://127.0.0.1:3000' + (socket.data.url || '/'));
                }

                // Execute serverHandler
                if (typeof socket.data.payload !== 'undefined' && socket.data.url.searchParams.get('stwHandler')) {
                    element = app.webbase.getElementById(socket.data.url.searchParams.get('stwHandler'));
                ...
                element = app.webbase.route(socket.data.url.pathname);
                if (!element)
                    ...
                    if (fragment) {
                        ...
                        socket.target.send(JSON.stringify({
                            ...
                            body: {
                                ...
                                searchParams: parse(socket.data.url.search.substring(1)), // Skip ?
